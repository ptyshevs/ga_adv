...
def save_image(adversary_example, adversary_score, filename):
...
class ImageGeneticSolver:
    def __init__(self, image_size, population_size=100, n_generations=1000, retain_best=0.8, retain_random=0.05, mutate_chance=0.05,
                 verbosity=0, verbose_step=50, random_state=None, warm_start=False, early_stopping=True, patience=100):
        """
        ...
        :note: same as GeneticSolver, but writes the best candidate so far on the hard drive.
        """
    ...
    def solve(self, fitness_fn, n_generations=-1, verbose_step=None, write_images=False, image_prefix=''):
        """
        ...
        for generation in range(self.n_generations):
            ...
            if self.verbosity:
                ...
                elif (generation % verbose_step == 0) or (generation == self.n_generations - 1):
                    print("Generation ",generation,": ",scores[0])
                    if write_images:
                        best_image = self._population[0]
                        best_score = scores[0]
                        save_image(best_image, best_score, f'{image_prefix}{generation:05}.png')
